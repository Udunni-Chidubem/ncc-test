...
tradersRouter.get("/help", async (req, res) => {
  ...
  let updateMessagestatus = await tradersController.updateMessagestatus(
    req,
    user_id
  );
...
tradersRouter.get("/getmessagescount", async (req, res) => {
  ...
  let getmessagescount = await tradersController.getmessagescount(
    req,
    trader.user_id
  );
...
tradersRouter.get("/dashboard", async (req, res) => {
  ...
  let transactionCount = await tradersController.getTransactionlogCount(
    trader.id
  );
  let transactions = await tradersController.getTransactions(trader.id);
...
tradersRouter.get("/checkout/preview", async (req, res) => {
  if (req.query.product) {
    ...
    let { getCartItems, trader, isVerified } =
      await tradersController.cartByProductId(req);
  ...
  try {
    ...
    if (paymentType == "card") {
      ...
      if (initial.status == true) {
        ...
        let { getCartItems, trader } =
          await tradersController.getCartItemsByIds(req, items);
        await tradersController.initializeTransaction(
          req,
          res,
          ref,
          getCartItems,
...
tradersRouter.post("/sales-sheet", async (req, res) => {
  ...
  let trader = await utils.getTraderPofile(user);
  let isVerified = await utils.isVerified(user);
  let sales_sheet_info = await tradersController.sales_sheet_info(
    req,
    res,
    user_id
  );
