module.exports = {
  ...
  loggedIn: async (req, res, next) => {
    if (req.isAuthenticated()) {
      ...
      if (role == "seed_trader") return res.redirect("seed-trader/dashboard");

      if (role == "seed_company") return res.redirect("seed-company/dashboard");
  ...
  adminPermission: async (req, res, next) => {
    ...
    // console.log(user.UserRole.Role)
    if (
      user.UserRole.Role.role_name == "admin" ||
      user.UserRole.Role.role_name == "nasc" ||
      user.UserRole.Role.role_name == "nigsims" ||
      user.UserRole.Role.role_name == "rra"
    ) {
      return next();
    } else {
      req.logOut();
