...
module.exports = {
  ...
  updatePassword: async (req, res) => {
    ...
    try {
      let sql =
        (await "update user set password='") +
        newpassword +
        "' where username = " +
        username +
        ";";
  ...
  editProfileData: async (req, res) => {
    ...
    try {
      ...
      const trader = await SeedTrader.update(
        data,
        {
          where: { user_id: user.id },
        },
      ...
      const existingDelivery = await DeliveryInformation.findOne({
        where: { user_id: user.id },
        attributes: ["user_id", "state_id", "lg_id", "address"],
        raw: true,
      });
  ...
  addToCart: async (req, res) => {
     ...
     */
    const isItemAlreadyAdded = await Cart.findOne({
      where: {
        [Op.and]: [
          {
            product_id: {
  ...
  createOrder: async (ids, transaction_log_id) => {
    let sql =
      "SELECT distinct s.id as company_id FROM cart c join product p on p.id = c.product_id join seedcompany s on s.user_id = p.user_id WHERE c.id IN (:ids)";
  ...
  orderStatus: async (req, res) => {
    let transaction_id = req.params.transaction_id;
    let sql =
      "select o.id as order_id ,p.description as description, p.product_name as product_name, p.file_name as" +
  ...
  getmessages: async (req, user_id) => {
    let sql =
      "SELECT * FROM messages where from_user =" +
      user_id +
      " or to_user =" +
      user_id +
      ";";
